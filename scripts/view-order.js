...
function populateOrderDetails(order) {
  ...
  document.getElementById("customer-id").textContent = `CUST-${order.customer_id}`;
  ...
  order.items.forEach((item) => {
    ...
    card.innerHTML = `
      ...
      <div class="item-card-body">
        <p><span class="item-card-label">Price:</span> EGP ${parseFloat(item.price).toFixed(2)}</p>
        <p><span class="item-card-label">Quantity:</span> ${item.quantity}</p>
        <p><span class="item-card-label">Total:</span> EGP ${itemTotal.toFixed(2)}</p>
      </div>
    `;
